...
class AlgoritmoGenetico:
    ...
    def __init__(self, 
                 ...
                 guardar_animacion: bool = False, # Nuevo parámetro
                 directorio_animacion: str = "animacion_ga" # Nuevo parámetro
                ):
        """
        ...
        Args:
            ...
            guardar_animacion: Si es True, guarda imágenes para animación.
            directorio_animacion: Directorio donde guardar las imágenes.
        """
        ...
        self.guardar_animacion = guardar_animacion
        self.directorio_animacion = directorio_animacion
        if self.guardar_animacion:
            os.makedirs(self.directorio_animacion, exist_ok=True)
            self.imagenes_animacion = [] # Para guardar los nombres de archivo
    ...
    def mutar(self, individuo: Individuo):
        ...
        if random.random() < self.prob_mutacion:
            ...
            # Asegurarse de que el individuo mutado esté dentro del rango
            individuo.genes[0] = max(self.rango[0], min(self.rango[1], individuo.genes[0]))
    ...
    def ejecutar(self) -> Tuple[Individuo, List[float], List[float]]:
        ...
        if self.guardar_animacion:
            self.generar_frame_animacion(0) # Guarda el frame inicial
        ...
        for generacion in range(1, self.max_generaciones + 1):
            ...
            if self.guardar_animacion:
                self.generar_frame_animacion(generacion)
        ...
        if self.guardar_animacion:
            self.crear_gif_animacion()
        
        return self.mejor_individuo, self.historial_fitness, self.historial_promedio
    
    def generar_frame_animacion(self, generacion: int):
        """
        Genera y guarda un frame de la animación mostrando la función objetivo
        y la distribución de la población actual.
        """
        fig, ax = plt.subplots(figsize=(8, 6)) # Un solo subplot para la animación
        
        x_vals = np.linspace(self.rango[0], self.rango[1], 500)
        y_vals = [self.funcion_objetivo(x) for x in x_vals]
        
        ax.plot(x_vals, y_vals, 'g-', linewidth=2, label='Función Objetivo')
        
        # Dibujar cada individuo como un punto
        poblacion_x = [ind.genes[0] for ind in self.poblacion]
        poblacion_y = [self.funcion_objetivo(x) for x in poblacion_x]
        ax.scatter(poblacion_x, poblacion_y, color='blue', alpha=0.6, s=50, label='Individuos')
        
        # Dibujar el mejor individuo de la generación actual
        ax.scatter([self.mejor_individuo.genes[0]], 
                   [self.funcion_objetivo(self.mejor_individuo.genes[0])],
                   color='red', marker='*', s=200, zorder=5, label='Mejor Global')

        ax.set_xlabel('x')
        ax.set_ylabel('f(x)')
        ax.set_title(f'Función Objetivo y Población (Generación {generacion})')
        ax.set_ylim(0, 800) # Mantener el eje y constante
        ax.set_xlim(self.rango[0], self.rango[1]) # Mantener el eje x constante
        ax.legend()
        ax.grid(True, alpha=0.3)
        
        # Guardar la imagen
        filename = os.path.join(self.directorio_animacion, f'generacion_{generacion:04d}.png')
        plt.savefig(filename)
        self.imagenes_animacion.append(filename)
        plt.close(fig) # Cierra la figura para no mostrarla y liberar memoria

    def crear_gif_animacion(self):
        """Une todas las imágenes de los frames en un GIF."""
        print(f"\nCreando GIF de animación en '{self.directorio_animacion}/animacion.gif'...")
        with imageio.get_writer(os.path.join(self.directorio_animacion, 'animacion.gif'), mode='I', duration=0.1) as writer:
            for filename in self.imagenes_animacion:
                image = imageio.imread(filename)
                writer.append_data(image)
        print("GIF creado exitosamente!")

        # Opcional: limpiar las imágenes individuales después de crear el GIF
        # for filename in self.imagenes_animacion:
        #     os.remove(filename)
        # os.rmdir(self.directorio_animacion) # Elimina el directorio si está vacío
...
if __name__ == "__main__":
    ...
    ag = AlgoritmoGenetico(
        ...
        guardar_animacion=True, # ¡Activado para generar la animación!
        directorio_animacion="animacion_ga_run1" # Un directorio para cada ejecución
    )
    
    # Ejecutar el algoritmo
    mejor, historial_fitness, historial_promedio = ag.ejecutar()
    
    # Graficar resultados (estáticos, como antes)
